...
function patchKeyedChildren(
  ...
  if (i > e1) {
    ...
    let moved = false // add
    let maxNewIndexSoFar = 0 // add
    const newIndexToOldIndexMap = new Array(toBePatched) // add
    // 数组补0
    for (let i = 0; i < toBePatched; i++) newIndexToOldIndexMap[i] = 0 // add
    ...
    for (let i = s1; i <= e1; i++) {
      ...
      if (newIndex === undefined) {
        ...
        if (newIndex >= maxNewIndexSoFar) { // add
          maxNewIndexSoFar = newIndex // add
        } else { // add
          moved = true // add
        } // add

        newIndexToOldIndexMap[newIndex - s2] = i + 1 // add
